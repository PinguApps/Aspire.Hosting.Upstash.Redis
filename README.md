...
Current state: the Aspire integration shape, public API shape, internal resource state model, Upstash Redis management client layer, Upstash Redis option/domain model, deploy-time parameter resolution, ownership-resolution decision engine, remote identity resolver, and immutable drift detector are implemented. The package extends the built-in Redis resource returned by `builder.AddRedis("cache")`, attaches internal Upstash deployment state with `.PublishToUpstash(...)`, resolves the configured deployment values from the Aspire deploy pipeline step, can resolve the intended remote identity by explicit database name, and can fail fast when an adopted database conflicts with create-time-only or unsafe settings. Later implementation tasks still own creation, mutable reconciliation, and deployed connection outputs.
