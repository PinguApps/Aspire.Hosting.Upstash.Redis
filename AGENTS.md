...
- Plan `5.2` is complete; `.PublishToUpstash(...)` attaches supplementary output references to the Redis resource, `GetUpstashRedisOutputs()` exposes stable `Endpoint`, `Port`, `Password`, `Tls`, and `DatabaseName` references, the deploy pipeline populates them from the credential-bearing database detail response after create/adopt/reconcile succeeds, and only `Password` is classified as secret.
