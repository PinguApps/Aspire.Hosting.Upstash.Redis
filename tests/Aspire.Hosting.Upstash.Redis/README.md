...
- `6.2`: covered by `Features/OwnershipModes/OwnershipDeployment.feature` for end-to-end deployment pipeline ownership semantics, including create-only, existing-only, create-or-adopt, strict repeated create-only failure, repeated deploy identity reuse for adopt-capable modes, duplicate-name and cached-identity edge cases, plus opt-in live create/adopt scenarios with isolated database deletion cleanup.
