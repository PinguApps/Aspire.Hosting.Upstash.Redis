...
- `4.1` and `4.2`: extend `Features/DeployReconcileOutputs` for create and reconcile.
- `4.3`: covered in `Features/DeployReconcileOutputs/ImmutableDrift.feature` for database-name identity, detectable platform, explicit primary-region, TLS-disabled failures, no unsafe mutation attempts, actionable wording, and mutable settings staying out of immutable drift.
