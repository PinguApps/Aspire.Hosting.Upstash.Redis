...
- `6.2`: covered by `Features/OwnershipModes/OwnershipDeployment.feature` for end-to-end deployment pipeline ownership semantics, including create-only, existing-only, create-or-adopt, repeated deploy identity reuse, duplicate-name and cached-identity edge cases, plus opt-in live create/adopt scenarios with isolated database deletion cleanup.
...
- `7.2`: covered in `Features/DocsSamples/AppHostSnippets.feature` by linking `samples/AppHostSnippets/UpstashRedisAppHostSnippets.cs` into the test project and invoking each snippet against a fresh Aspire builder.
- `7.1`: align README snippets to `samples/AppHostSnippets/UpstashRedisAppHostSnippets.cs` where practical and add DocsSamples coverage only for README-specific drift checks that are not already covered by the compiled sample source.
- `8.2`: planned in `docs/typescript-apphost-acceptance.md` as the TypeScript AppHost acceptance matrix, fixture topology, docs IA, demo definition, and final human walkthrough checklist.
