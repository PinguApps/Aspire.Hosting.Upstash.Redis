...
- `Fixtures/TypeScriptAppHost` is the checked-in TypeScript AppHost validation fixture. Generated `.aspire/modules/` content is regenerated by Aspire CLI flows and must not be committed.
- Normal CI excludes `@live-upstash` scenarios and runs the NuGet-backed TypeScript AppHost package gate. The mandatory, credential-gated, and manual release split is documented in `docs/ci-release-gates.md`.
...
- `10.1`: covered in `Features/ApiShape/TypeScriptAppHostFixture.feature` by validating the checked-in TypeScript AppHost fixture consumes the intended generated module surface, calls `publishToUpstash`, consumes supplementary outputs, preserves normal Redis references, restores the generated SDK, type-checks the generated surface, starts locally with Redis readiness handling, lists publish steps, and keeps generated modules out of source control. `Features/DeployReconcileOutputs/TypeScriptDeployment.feature` adds fake-provider repeated-deploy and gated `@live-upstash` validation through the TypeScript bridge and existing deployment pipeline.
