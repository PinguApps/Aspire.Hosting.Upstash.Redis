...
- `4.1` and `4.3`: extend `Features/DeployReconcileOutputs` for create and immutable drift failures.
- `4.2`: covered in `Features/DeployReconcileOutputs/ReconcileMutableSettings.feature` for no-op reconcile, deterministic read-regions/plan/budget/eviction update order, explicit-setting-only enforcement, provider mutation failures, and convergence failures.
